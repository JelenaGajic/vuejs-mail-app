<template>
    <div class="inbox-body">
        <div class="btn btn-primary" @click="navigateBack">
            <i class="fa fa-arrow-left" aria-hidden="true"></i>&nbsp; Back
        </div>
        <p><strong>Date: </strong>{{ data.message.date.fromNow() }}</p>
        <p><strong>Date: </strong>{{ data.message.from.name }} {{ data.message.from.email }} </p>
        <hr>
        ...
        <div v-if="data.message.attachments.length > 0" class="attachments">
            ...
            <ul>
                <li v-for="(attachment, ind) in data.message.attachments" :key="ind">
                    <i class="fa fa-paperclip"></i> {{ attachment.fileName}} ({{ attachment.size | foramtBytes }})
                </li>
            </ul>
        </div>
    </div>
